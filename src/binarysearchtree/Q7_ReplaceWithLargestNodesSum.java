...
public class Q7_ReplaceWithLargestNodesSum {
    public static void main(String[] args) {
//        BinaryTreeNode<Integer> root = takeInputBetter(true, 0, false);
        int[] inOrder = {1, 2, 3, 4, 5, 6, 7};
        int[] preOrder = {4, 2, 1, 3, 6, 5, 7};
        BinaryTreeNode<Integer> root = buildTree(preOrder, inOrder);
        replaceWithLargestNodesSum(root);
        printDetailed(root);
    }

    // TODO: method to replace with largest nodes sum
    public static void replaceWithLargestNodesSum(BinaryTreeNode<Integer> root) {
        int sum = 0;
        replaceWithLargestNodesSum(root, sum);
    }

    public static int replaceWithLargestNodesSum(BinaryTreeNode<Integer> root, int sum) {
        if (root == null) {
            return sum;
        }

        sum = replaceWithLargestNodesSum(root.right, sum);
        sum = sum + root.data;
        root.data = sum;
        sum = replaceWithLargestNodesSum(root.left, sum);
        return sum;
    }
