...
//        BinaryTreeNode<Integer> root = takeInputBetter(true, 0, false);
        ...
        printDetailed(root);
        ArrayList<Integer> arr = getPath(root, 50);
        if (arr == null) {
    ...
    // TODO: method to path of targeted node or value
    public static ArrayList<Integer> getPath(BinaryTreeNode<Integer> root, int target) {
        ...
        if (root.data == target) {
            ...
            ArrayList<Integer> list = getPath(root.left, target);
            if (list != null) {
                list.add(root.data);
                return list;
            }
        ...
        if (root.data < target) {
            list = getPath(root.right, target);
            if (list != null) {
                list.add(root.data);
                return list;
            }
